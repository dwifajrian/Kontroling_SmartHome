//include library wifi nodemcu esp8266
#include <ESP8266HTTPClient.h>
#include <ESP8266WiFi.h>

//Network SSID
const char* ssid = "TP-Link_Extender";
const char* pass = "1sampai11";

//ambil IP server
const char* host = "192.168.1.12";

//pin relay
#define pin_relay 5 //pin GPIO = D1
#define pin_relayy 4 //pin GPIO = D2

void setup() {
  Serial.begin(9600);

  //koneksi ke WiFi
  WiFi.hostname("NodeMcu");
  WiFi.begin(ssid, pass);

  //cek koneksi berhasil atau tidak
  while(WiFi.status() !=WL_CONNECTED)
  {
    //coba koneksi terus
    Serial.print(".");
    delay(500);
  }
  //apabila terkoneksi
  Serial.print("Berhasil Konek");

  //pin relay
  pinMode(pin_relay, OUTPUT);
  //status set awal
  digitalWrite(pin_relay, LOW);  //High= 1, Low = 0

  //pin relay 2
  pinMode(pin_relayy, OUTPUT);
  //status set awal
  digitalWrite(pin_relayy, LOW);  //High= 1, Low = 0
}

void loop() {
  //koneksi ke web server
  WiFiClient client;
  const int httpPort = 80;
  //uji koneksi
  if(!client.connect(host, httpPort))
  {
    Serial.println("Gagal Koneksi ke Server");
    return;
  }
  //berhasil konek
  //Serial.println("Berhasil Koneksi Ke Server");

  //baca status relay
  String LinkRelay ;
  HTTPClient httprelay ;
  LinkRelay = "http://" + String(host) + "/controlling/bacarelay.php";
  httprelay.begin(LinkRelay);
  //ambil isi status relay
  httprelay.GET();
  //baca status respon relay
  String statusrelay = httprelay.getString();  
  Serial.println(statusrelay);
  httprelay.end();

  //ubah status releay di nodemcu
  digitalWrite(pin_relay, statusrelay.toInt());

  //baca status relay 2
  String LinkRelayy ;
  HTTPClient httprelayy ;
  LinkRelayy = "http://" + String(host) + "/controlling/bacarelayy.php";
  httprelayy.begin(LinkRelayy);
  //ambil isi status relay
  httprelayy.GET();
  //baca status respon relay
  String statusrelayy = httprelayy.getString();  
  Serial.println(statusrelayy);
  httprelayy.end();

  //ubah status releay di nodemcu
  digitalWrite(pin_relayy, statusrelayy.toInt());
  
}
